...
function setDisplayPage() {
    const columnCount = parseInt(getComputedStyle(document.querySelector('body')).getPropertyValue('column-count'))
    if (1 < columnCount) { document.querySelector(':root').style.setProperty('--display-page', `inline`) }
    else { document.querySelector(':root').style.setProperty('--display-page', `none`) }
}
