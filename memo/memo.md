...
　これを利用して、表示領域サイズに応じてカラムの先頭となる要素をがんばって計算し、その要素の`class`属性値に`break`を付与すれば、カラムの先頭を指定できる。その位置に[scrollIntoView()][]すれば、座標の誤差なくスクロールできるはず。
...
　今回はCSSの`column-count`でなくHTMLの[`<iframe>`][]による分割が最適ではないか。

[`<iframe>`]:https://developer.mozilla.org/ja/docs/Web/HTML/Element/iframe

　画面の左をつぶやき入力・設定などの入力フォーム領域とし、右をつぶやき一覧にする。それぞれ個別に縦スクロールバーをもっている。こうした構成が正解な気がしてきた。

　もし`column-count`だと入力フォームとつぶやき一覧に分割できない。たとえば一覧をずっと下まで見たあと、入力フォームで投稿したくなったとき、最上端までスクロールせねばならない。また、入力フォームが長いと右にもフォームが表示される。

　そうした事態をふせぐためには[`<iframe>`][]による分割が有効だろう。
